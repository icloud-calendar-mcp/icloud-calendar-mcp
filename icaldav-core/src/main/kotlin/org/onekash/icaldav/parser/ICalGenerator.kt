...
class ICalGenerator(
     ...
     * @param includeMethod Include METHOD:PUBLISH (some CalDAV servers reject this
     *                      for PUT operations - set to false for CalDAV)
    ...
    private fun StringBuilder.appendVEvent(
        ...
        // Status: emitted unconditionally for iCloud compatibility (a missing
        // STATUS triggers HTTP 400). A null model status (no STATUS in the source)
        // defaults to CONFIRMED here per common practice, without polluting the
        // parsed model — see ICalEvent.status.
        crlfLine("STATUS:${(event.status ?: EventStatus.CONFIRMED).toICalString()}")
    ...
    private fun StringBuilder.appendVAlarm(alarm: ICalAlarm) {
        ...
        // Description: RFC 5545 §3.6.6 makes DESCRIPTION mandatory for both DISPLAY
        // (text to show) and EMAIL (message body). AUDIO has no DESCRIPTION.
        if (alarm.action == AlarmAction.DISPLAY || alarm.action == AlarmAction.EMAIL) {
            crlfLine("DESCRIPTION:${alarm.description ?: "Reminder"}")
        }

        // Summary: RFC 5545 §3.6.6 makes SUMMARY mandatory for EMAIL (message
        // subject). Emit whenever provided; default it for EMAIL so the required
        // property is never dropped.
